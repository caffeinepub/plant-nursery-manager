        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            {loading ? (
              ...
              <p
                className={`text-2xl font-bold ${positive === undefined ? "text-foreground" : positive ? "text-primary" : "text-accent"}`}
              >
                {value}
              </p>
            )}
          ...
          <div
            className={`p-2.5 rounded-xl ${positive === undefined ? "bg-muted text-muted-foreground" : positive ? "bg-primary/10 text-primary" : "bg-accent/10 text-accent"}`}
          >
...
export default function ProfitAnalysis() {
  ...
  const { data: salesRecords = [], isLoading: salesLoading } =
    useGetAllSalesRecords();
  const { data: expenditures = [], isLoading: expLoading } =
    useGetAllExpenditures();
  ...
  const analysis = useMemo(() => {
    ...
    const totalExpenditure = filteredExp.reduce(
  ...
  const chartData = useMemo(() => {
    ...
    const dateMap = new Map<
      string,
      { date: string; revenue: number; expenditure: number }
    >();
  ...
  const handleExportExcel = async () => {
    ...
    try {
      const summaryRows = [
        { Metric: "Total Revenue", "Amount (₹)": analysis.totalRevenue },
        { Metric: "Total COGS", "Amount (₹)": analysis.totalCOGS },
        { Metric: "Gross Profit", "Amount (₹)": analysis.grossProfit },
        {
          Metric: "Total Expenditure",
          "Amount (₹)": analysis.totalExpenditure,
        },
        { Metric: "Net Profit", "Amount (₹)": analysis.netProfit },
      ];

      const breakdownRows = chartData.map((d) => ({
        Period: d.date,
        "Revenue (₹)": d.revenue,
        "Expenditure (₹)": d.expenditure,
        "Net Profit (₹)": d.revenue - d.expenditure,
      }));

      await exportToExcel(`profit-report-${startDate}-to-${endDate}.xlsx`, {
        Summary: summaryRows,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        ...
        <MetricCard
          ...
          subtitle={
            analysis.netProfit < 0 ? "⚠ Net Loss" : "After all expenses"
          }
          icon={Leaf}
